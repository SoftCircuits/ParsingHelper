...
#pragma warning restore MSTEST0032 // Assertion condition is always true
            ...
            Assert.IsFalse(helper.EndOfText);
            ...
            Assert.IsTrue(helper.EndOfText);
        ...
        public void ParseTests()
        {
            ...
            Assert.AreSequenceEqual(parseAllResults, [.. helper.ParseTokens(' ', '\t', '\r', '\n', '.')]);

            helper.Reset();
            Assert.AreSequenceEqual(parseAllResults, [.. helper.ParseTokens(c => " \t\r\n.".Contains(c))]);
        ...
        public void MatchesCurrentPositionTests()
        {
            ...
            Assert.IsTrue(helper.MatchesCurrentPosition("consecrated it"));
            Assert.IsTrue(helper.MatchesCurrentPosition("CONSECRATED IT", StringComparison.OrdinalIgnoreCase));
            Assert.IsFalse(helper.MatchesCurrentPosition(string.Empty));
            Assert.IsFalse(helper.MatchesCurrentPosition(string.Empty, StringComparison.OrdinalIgnoreCase));
            Assert.IsFalse(helper.MatchesCurrentPosition("consecrated_it"));
            Assert.IsFalse(helper.MatchesCurrentPosition("CONSECRATED_IT", StringComparison.OrdinalIgnoreCase));

            Assert.IsTrue(helper.MatchesCurrentPosition((char[])['c', 'o', 'n', 's', 'e', 'c', 'r', 'a', 't', 'e', 'd', ' ', 'i', 't']));
            Assert.IsFalse(helper.MatchesCurrentPosition((char[])['o', 'n', 's', 'e', 'c', 'r', 'a', 't', 'e', 'd', ' ', 'i', 't']));

            helper.Index = LongTest.Length - 1;
            Assert.IsFalse(helper.MatchesCurrentPosition("consecrated it"));
            Assert.IsFalse(helper.MatchesCurrentPosition("CONSECRATED IT", StringComparison.OrdinalIgnoreCase));
...
people, for the people, shall not perish from the earth.", helper.Extract(start));
            ...
            Assert.Throws<ArgumentOutOfRangeException>(() => helper[0..4]);

            helper.Reset(string.Empty);
            Assert.Throws<ArgumentOutOfRangeException>(() => helper[0..1]);
        }
...
people, for the people, shall not perish from the earth.", helper.ExtractAsSpan(start).ToString());
            ...
            Assert.Throws<ArgumentOutOfRangeException>(() => helper.ExtractAsSpan(0, 4));
...
#pragma warning restore IDE0054 // Use compound assignment
        ...
        public void RegExTests()
        {
            ...
            Assert.AreSequenceEqual(StringArray, [.. results]);
            ...
            Assert.AreSequenceEqual([], [.. results]);
            Assert.AreEqual(text.Length, helper.Index);
            ...
            Assert.IsTrue(helper.SkipTo("123"));
            helper.SkipRegEx(@"\d+");
        ...
        public void ParseLineTests()
        {
            ...
            foreach (var test in tests)
            {
                ...
                while (helper.ParseLine(out string line))
                    lines.Add(line);
                Assert.AreSequenceEqual(test.Item2, lines);
            }
